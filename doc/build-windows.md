...
Below are some notes on how to build PlexHive Core for Windows.

The options known to work for building PlexHive Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the PlexHive Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the PlexHive Core applications.
...
Note that for WSL the PlexHive Core source path MUST be somewhere in the default mount file system, for
example /usr/src/plexhive, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/plexhive-project/plexhive.git
    sudo chmod -R a+rw plexhive
...
way. This will install to `c:\workspace\plexhive`, for example:

    make install DESTDIR=/mnt/c/workspace/plexhive
...
causes two of the plexhive executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the plexhive code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the plexhive source code).
