...
def main():
    ...
    tmpdir = "%s/plexhive_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and plexhived must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "plexhived"]) is not None:
            print("%sWARNING!%s There is already a plexhived process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "PLEXHIVED" not in os.environ:
        os.environ["PLEXHIVED"] = build_dir + '/src/plexhived' + exeext
        os.environ["PLEXHIVECLI"] = build_dir + '/src/plexhive-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `plexhive-cli help` (`rpc_interface.txt`).
