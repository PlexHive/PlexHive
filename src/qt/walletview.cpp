...
#include <qt/hivedialog.h>      // PlexHive: Hive page
...
#include <boost/thread.hpp>     // PlexHive: Key import helper
#include <wallet/rpcwallet.h>   // PlexHive: Key import helper
#include <wallet/wallet.h>      // PlexHive: Key import helper
#include <validation.h>         // PlexHive: Key import helper
...
#include <QPushButton>
#include <QVBoxLayout>
#include <QInputDialog>         // PlexHive: Key import helper
    ...
    platformStyle(_platformStyle)
{
    ...
    hivePage = new HiveDialog(platformStyle); // PlexHive: Hive page
    ...
    addWidget(hivePage);   // PlexHive: Hive page
...
void WalletView::setBitcoinGUI(BitcoinGUI *gui)
{
    if (gui)
    {
        ...
        // PlexHive: Hive: Go to hive page if bee button on overview clicked
        ...
        // PlexHive: Hive: Connect hive status update signal
...
void WalletView::setClientModel(ClientModel *_clientModel)
{
    ...
    hivePage->setClientModel(_clientModel); // PlexHive: Hive page
}
...
void WalletView::setWalletModel(WalletModel *_walletModel)
{
    ...
    hivePage->setModel(_walletModel);         // PlexHive: Hive page
    ...
    if (_walletModel)
    {
		...
		// PlexHive: Hive: Ask for passphrase if needed hive only
...
// PlexHive: Hive: Unlock wallet just for hive
...
{
    bool ok;
    QString privKey = QInputDialog::getText(0, tr(PACKAGE_NAME), tr("Enter a PlexHive private key to import into your wallet."), QLineEdit::Normal, "", &ok);
    if (ok && !privKey.isEmpty()) {
        ...
        if (!vchSecret.SetString(privKey.toStdString())) {
            QMessageBox::critical(0, tr(PACKAGE_NAME), tr("This doesn't appear to be a PlexHive private key."));
            return;
        }
