...
#include <qt/optionsdialog.h> // PlexHive: Hive: Mining optimisations
...
void HiveDialog::updateData(bool forceGlobalSummaryUpdate) {
    ...
    if(model && model->getHiveTableModel()) {
        ...
        if (clientModel && clientModel->getNumConnections() == 0) {
            tooltip = "PlexHive is not connected";
...
void HiveDialog::on_createBeesButton_clicked() {
    if (model) {
        ...
        model->createBees(ui->beeCountSpinner->value(), clientModel->getOptionsModel()->getHiveContribCF(), this, beePopIndex); // PlexHive: MinotaurX+Hive1.2
    }
}

// PlexHive: Hive: Mining optimisations: Shortcut to Hive mining options
