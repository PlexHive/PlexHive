...
const std::string strMessageMagic = "PlexHive Signed Message:\n";   // PlexHive: Should still use LTC's strMessageMagic so that pre-fork sigs validate
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s PLHV additional fees, %d delta bytes\n",
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // PlexHive: Hive: Check PoW or Hive work depending on blocktype
...
{
    // PlexHive: Issue premine on 1st post-fork block
    if (nHeight == consensusParams.lastScryptBlock + 1)
        return consensusParams.premineAmount * COIN * COIN_SCALE;

    int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    // PlexHive: Force block reward to zero when right shift is undefined, and don't attempt to issue past total money supply
    ...
    // PlexHive: Slow-start the first n blocks to prevent early miners having an unfair advantage
...
// PlexHive: Hive: Return the current cost for a single worker bee
CAmount GetBeeCost(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // PlexHive: MinotaurX+Hive1.2: Note that this doesn't change; bee cost remains calculated against the base subsidy. Great!
...
void ThreadScriptCheck() {
    RenameThread("plexhive-scriptch");
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    // PlexHive: MinotaurX+Hive1.2: Set bit 29 to 0
...
public:
    ...
    {
        // PlexHive: MinotaurX+Hive1.2: Versionbits always active since powforktime and high bits repurposed at minotaurx UASF activation;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    // PlexHive: Enforce use of correct fork ID
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // PlexHive: MinotaurX+Hive1.2: Get correct block reward
    ...
    // PlexHive: Ensure that lastScryptBlock+1 coinbase TX pays to the premine address
    if (pindex->nHeight == chainparams.GetConsensus().lastScryptBlock + 1) {
        if (block.vtx[0]->vout[0].scriptPubKey.size() == 1) {
            ...
            // Allow the block to be mined without validating the scriptPubKey
            LogPrintf("ConnectBlock(): allowing mine without scriptPubKey validation\n");
        }
    }
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            // PlexHive: MinotaurX+Hive1.2: Mask out blocktype before checking for possible unknown upgrade
            if (IsMinotaurXEnabled(pindex, chainParams.GetConsensus())) {
                ...
                // PlexHive: Hive: Don't warn about unexpected version in Hivemined blocks
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // PlexHive: Hive: Check Hive proof
    if (block.IsHiveMined(consensusParams))
        if (!CheckHiveProof(&block, consensusParams))
    ...
    // Check transactions
    for (const auto& tx : block.vtx)
        if (!CheckTransaction(*tx, state, true))    // PlexHive: Fix CVE-2018-17144
...
// PlexHive: Hive: Check if Hive is activated at given point
...
// PlexHive: Hive: Check if Hive 1.1 is activated at given point
...
// PlexHive: MinotaurX+Hive1.2: Check if MinotaurX is activated at given point
...
// PlexHive: Hive: Get the well-rooted deterministic random string (see whitepaper section 4.1)
...
// PlexHive: Hive: Get tx by given hash, from a block at given chain height
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // PlexHive: Hive: Check appropriate Hive or PoW target
    ...
    if (block.IsHiveMined(consensusParams)) {
        ...
        // PlexHive: MinotaurX+Hive1.2: Handle pow type
    ...
    // Check timestamp
    // PlexHive: MinotaurX+Hive1.2: Use alternative MAX_FUTURE_BLOCK_TIME after fork
    ...
    // PlexHive: MinotaurX+Hive1.2: Handle nVersion differently after activation
    if (!IsMinotaurXEnabled(pindexPrev,consensusParams)) {
