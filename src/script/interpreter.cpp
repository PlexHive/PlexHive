...
bool static IsDefinedHashtypeSignature(const valtype &vchSig) {
    ...
    unsigned char nHashType = vchSig[vchSig.size() - 1] & (~(SIGHASH_ANYONECANPAY | SIGHASH_FORKID));   // PlexHive: Use SIGHASH_FORKID
...
// PlexHive: Drop the signature in scripts when SIGHASH_FORKID is not used
...
// PlexHive: Check fork ID before calling FindAndDelete
...
bool CheckSignatureEncoding(const std::vector<unsigned char> &vchSig, unsigned int flags, ScriptError* serror) {
    ...
    if ((flags & (SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_STRICTENC)) != 0 && !IsValidSignatureEncoding(vchSig)) {
        ...
        // serror is set
        return false;
    } else if ((flags & SCRIPT_VERIFY_STRICTENC) != 0) {            // PlexHive: Enforce fork ID checking
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    if (sigversion == SIGVERSION_BASE) {
                        CleanupScriptCode(scriptCode, vchSig, flags);   // PlexHive: Check fork ID before calling FindAndDelete
                    }
