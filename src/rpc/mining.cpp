 ...
 */
// PlexHive: Hive: count hashes with dedicated function, dont use chainwork. GetNumHashes is Hive Aware.
// PlexHive: MinotaurX+Hive1.2: Only consider the correct powType when counting hashes
UniValue GetNetworkHashPS(int lookup, int height, POW_TYPE powType) {
    ...
    if (lookup <= 0)
        lookup = IsHive11Enabled(pb, Params().GetConsensus()) ? 1 : pb->nHeight % Params().GetConsensus().DifficultyAdjustmentInterval() + 1;   // PlexHive: Hive 1.1: Taking the opportunity to provide a more sensible default.

    // If lookup is larger than chain, then set it to chain length.
    if (lookup > pb->nHeight)
        lookup = pb->nHeight;

    // PlexHive: MinotaurX+Hive1.2: Skip incorrect powType
	...
	arith_uint256 workDiff = GetNumHashes(*pb, powType);    // PlexHive: MinotaurX+Hive1.2: add powType param
    ...
    for (int i = 0; i < lookup; i++) {
        ...
        workDiff += GetNumHashes(*pb, powType);              // PlexHive: MinotaurX+Hive1.2: add powType param
    }
...
// PlexHive: Hive: Mining optimisations: Set hive mining params
...
// PlexHive: Hive: Mining optimisations: Get hive mining params
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated plexhive to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when plexhived is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
    ...
    if (IsMinotaurXEnabled(chainActive.Tip(), Params().GetConsensus()))
        obj.push_back(Pair("minotaurxdifficulty", GetDifficulty(nullptr, false, POW_TYPE_MINOTAURX)));    // PlexHive: MinotaurX+Hive1.2
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        // PlexHive: MinotaurX+Hive1.2: Include help note about algo field in template_request
        throw std::runtime_error(
    ...
    std::string strAlgo = gArgs.GetArg("-powalgo", DEFAULT_POW_TYPE);   // PlexHive: MinotaurX+Hive1.2: Pow type (as string)
    if (!request.params[0].isNull())
    {
        ...
        // PlexHive: MinotaurX+Hive1.2: Override default pow type
    ...
    // PlexHive: MinotaurX+Hive1.2: Check for a valid pow type
    bool algoFound = false;
    POW_TYPE powType;
    for (unsigned int i = 0; i < NUM_BLOCK_TYPES; i++) {
        if (strAlgo == POW_TYPE_NAMES[i]) {
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PlexHive is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PlexHive is downloading blocks...");
    ...
    static POW_TYPE lastPowType = NUM_BLOCK_TYPES;  // PlexHive: MinotaurX+Hive1.2
    if (pindexPrev != chainActive.Tip() ||
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5) ||
        fLastTemplateSupportsSegwit != fSupportsSegwit ||
        lastPowType != powType) // PlexHive: MinotaurX+Hive1.2: Include powType check in cache refresh condition
    {
        ...
        pblocktemplate = BlockAssembler(Params()).CreateNewBlock(scriptDummy, fSupportsSegwit, nullptr, powType);   // PlexHive: MinotaurX+Hive1.2: Include powType
        lastPowType = powType;   // PlexHive: MinotaurX+Hive1.2: Cache pow type just requested
        if (!pblocktemplate)
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart plexhived with -deprecatedrpc=estimatefee.\n"
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "mining",             "sethiveparams",          &sethiveparams,          {"hivecheckdelay", "hivecheckthreads", "hiveearlyout"} },  // PlexHive: Hive: Mining optimisations: Set hive mining params
    { "mining",             "gethiveparams",          &gethiveparams,          {} },  // PlexHive: Hive: Mining optimisations: Get hive mining params
};
