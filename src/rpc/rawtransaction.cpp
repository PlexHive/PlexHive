...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the plexhive address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
// PlexHive: Hive: Create a raw bee creation transaction allowing user to specify the inputs
UniValue createrawbct(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "3. \"honey_address\"        (string, required) The PLHV address to receive rewards for blocks mined by bee(s) created in this transaction.\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) plexhive address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    int nHashType = SIGHASH_ALL | SIGHASH_FORKID;																// PlexHive: Replay attack protection
    if (!request.params[3].isNull()) {
        static std::map<std::string, int> mapSigHashValues = {
            ...
            {std::string("ALL|FORKID"), int(SIGHASH_ALL|SIGHASH_FORKID)},											// PlexHive: Replay attack protection
            {std::string("NONE|FORKID"), int(SIGHASH_NONE|SIGHASH_FORKID)},											// PlexHive: Replay attack protection
            {std::string("SINGLE|FORKID"), int(SIGHASH_SINGLE|SIGHASH_FORKID)},										// PlexHive: Replay attack protection
            {std::string("ALL|FORKID|ANYONECANPAY"), int(SIGHASH_ALL|SIGHASH_FORKID|SIGHASH_ANYONECANPAY)},			// PlexHive: Replay attack protection
            {std::string("NONE|FORKID|ANYONECANPAY"), int(SIGHASH_NONE|SIGHASH_FORKID|SIGHASH_ANYONECANPAY)},		// PlexHive: Replay attack protection
            {std::string("SINGLE|FORKID|ANYONECANPAY"), int(SIGHASH_SINGLE|SIGHASH_FORKID|SIGHASH_ANYONECANPAY)},	// PlexHive: Replay attack protection            
        };
        std::string strHashType = request.params[3].get_str();
        if (mapSigHashValues.count(strHashType))
            nHashType = mapSigHashValues[strHashType] | SIGHASH_FORKID;												// PlexHive: Replay attack protection
        else
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "rawtransactions",    "createrawbct",           &createrawbct,           {"inputs","beecount","honey_address","community_contrib", "locktime"} }, // PlexHive: Hive
