...
#include <primitives/block.h>       // PlexHive: MinotaurX+Hive1.2: for POW_TYPE
 ...
 */
// PlexHive: Hive: Optional getHiveDifficulty param
// PlexHive: MinotaurX+Hive1.2: Add powType param
double GetDifficulty(const CChain& chain, const CBlockIndex* blockindex, bool getHiveDifficulty = false, POW_TYPE powType = POW_TYPE_SHA256)
{
    ...
    // PlexHive: Hive: If tip is PoW and we want hivemined, step back until we find a Hive block
    // PlexHive: Hive 1.1: Allow there to be multiple hive blocks in the way
    if (getHiveDifficulty) {
        ...
        // PlexHive: MinotaurX+Hive1.2: Skip over incorrect powTypes
...
// PlexHive: Hive: Pass through optional getHiveDifficulty param
// PlexHive: MinotaurX+Hive1.2: Add additional POW_TYPE arg
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    int confirmations = -1;
    auto consensusParams = Params().GetConsensus();     // PlexHive: MinotaurX+Hive1.2

    // Only report confirmations if the block is on the main chain
    if (chainActive.Contains(blockindex))
        confirmations = chainActive.Height() - blockindex->nHeight + 1;
    bool isHive = blockindex->GetBlockHeader().IsHiveMined(consensusParams);    // PlexHive: MinotaurX+Hive1.2
    result.push_back(Pair("type", isHive ? "hive" : "pow")); // PlexHive: Hive 1.1: Show block type in JSON
    // PlexHive: MinotaurX+Hive1.2: Only report powtype for pow blocks
    if (!isHive)
    ...
    if (IsMinotaurXEnabled(blockindex, consensusParams))
        result.push_back(Pair("minotaurxdifficulty", GetDifficulty(blockindex, false, POW_TYPE_MINOTAURX)));    // PlexHive: MinotaurX+Hive1.2
    result.push_back(Pair("hivedifficulty", GetDifficulty(blockindex, true)));  // PlexHive: Hive
...
// PlexHive: MinotaurX+Hive1.2: Allow additional powalgo arg
...
// PlexHive: Hive: Get hive difficulty
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of plexhive addresses\n"
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "blockchain",         "gethivedifficulty",      &gethivedifficulty,      {} },        // PlexHive: Get Hive difficulty
