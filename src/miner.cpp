...
#include <wallet/wallet.h>  // PlexHive: Hive
#include <rpc/server.h>     // PlexHive: Hive
#include <base58.h>         // PlexHive: Hive
#include <sync.h>           // PlexHive: Hive
#include <boost/thread.hpp> // PlexHive: Hive: Mining optimisations
#include <crypto/minotaurx/yespower/yespower.h>  // PlexHive: MinotaurX+Hive1.2


static CCriticalSection cs_solution_vars;
std::atomic<bool> solutionFound;            // PlexHive: Hive: Mining optimisations: Thread-safe atomic flag to signal solution found (saves a slow mutex)
std::atomic<bool> earlyAbort;               // PlexHive: Hive: Mining optimisations: Thread-safe atomic flag to signal early abort needed
CBeeRange solvingRange;                     // PlexHive: Hive: Mining optimisations: The solving range (protected by mutex)
uint32_t solvingBee;                        // PlexHive: Hive: Mining optimisations: The solving bee (protected by mutex)
...
void BlockAssembler::resetBlock()
{
    ...
    fIncludeBCTs = true;    // PlexHive: Hive
...
// PlexHive: Hive: If hiveProofScript is passed, create a Hive block instead of a PoW block
// PlexHive: MinotaurX+Hive1.2: Accept POW_TYPE arg
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, const CScript* hiveProofScript, const POW_TYPE powType)
{
    ...
    // PlexHive: Hive: Make sure Hive is enabled if a Hive block is requested
    ...
    // PlexHive: MinotaurX+Hive1.2: Refuse to attempt to create a non-sha256 block before activation
    ...
    // PlexHive: MinotaurX+Hive1.2: If MinotaurX is enabled, and we're not creating a Hive block, encode desired pow type.
    ...
    // PlexHive: Don't include BCTs in hivemined blocks
    if (hiveProofScript)
        fIncludeBCTs = false;
    ...
    // PlexHive: Hive: Create appropriate coinbase tx for pow or Hive block
    if (hiveProofScript) {
        ...
        // PlexHive: MinotaurX+Hive1.2: Hive rewards are 150% of base block reward
        ...
        // PlexHive: MinotaurX+Hive1.2: Pow rewards are 50% of base block reward
    ...
    // PlexHive: Hive: Choose correct nBits depending on whether a Hive block is requested
    if (hiveProofScript)
        pblock->nBits = GetNextHiveWorkRequired(pindexPrev, chainparams.GetConsensus());
    else {
        // PlexHive: MinotaurX+Hive1.2: If MinotaurX is enabled, handle nBits with pow-specific diff algo
        if (IsMinotaurXEnabled(pindexPrev, chainparams.GetConsensus()))
    ...
    // PlexHive: Hive: Set nonce marker for hivemined blocks
...
{
    const Consensus::Params& consensusParams = Params().GetConsensus(); // PlexHive: Hive
    ...
    for (const CTxMemPool::txiter it : package) {
        ...
        if (!fIncludeWitness && it->GetTx().HasWitness())
            return false;
        // PlexHive: Hive: Inhibit BCTs if required
...
// PlexHive: Hive: Bee management thread
void BeeKeeper(const CChainParams& chainparams) {
    ...
    try {
        while (true) {
            // PlexHive: Hive: Mining optimisations: Parameterised sleep time
...
// PlexHive: Hive: Mining optimisations: Thread to signal abort on new block
...
// PlexHive: Hive: Mining optimisations: Thread to check a single bin
...
// PlexHive: MinotaurX+Hive1.2: Thread to check a single bee bin
...
// PlexHive: Hive: Attempt to mint the next block
bool BusyBees(const Consensus::Params& consensusParams, int height) {
    ...
    int binID = 0;
    bool minotaurXEnabled = IsMinotaurXEnabled(pindexPrev, consensusParams);    // PlexHive: MinotaurX+Hive1.2: Check if minotaurX enabled
    while (beeBinIterator != beeBins.end()) {
        ...
        // PlexHive: MinotaurX+Hive1.2: Use correct inner hash
        if (!minotaurXEnabled)
